...
interface AsymmetricalSectionProps {
  ...
  className?: string;
  key?: string | number;
  imageWidth?: string; // e.g. "md:w-3/5"
}
  ...
  return (
    <section ref={ref} className={cn("py-48 px-8 overflow-hidden", className)}>
      <div className={cn(
        ...
        <div className={cn("w-full space-y-6 mix-blend-difference text-white", textWidth)}>
          <motion.span 
          ...
          <TextReveal 
            text={title} 
            className="text-6xl md:text-8xl massive-text leading-none text-white" 
          />
          <motion.p
